...
#
# We are going to use it by first setting all the other col classes to "NULL"
...
#
# Since the participants we divided into "train" and "test" groups, I can read test in
...
#
# We now need to do subjects and actions, and add them to x_df
...
#
# Join into one DF and make a little easier to work with using dplyr (tbl_df)
...
#
#
# From the data set above (mytable), creates a second, independent tidy data set with the average
# of each variable for each activity and each subject. We use the summarise_each function to
# reduce each group to a single row, mutating the columns to find the mean of each activity in the
# process.
#
subs<-group_by(mytable,Subject,Activity)
tidy<-summarise_each(subs,funs(mean))
# We then write the tidied data file results to tidy.txt.
